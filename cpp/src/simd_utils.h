...
struct SimdType<SSE>{
  ...
  static std::vector<float> registerToVectorWrapper(Func&& func, Args&&... args);
...
template<>
typename SimdType<SSE>::register_t PORTABLE_TARGET_SSE SimdType<SSE>::loadAndConvertToFloat(float const* data){
    return _mm_loadu_ps(data);
}

template<>
typename SimdType<AVX2>::register_t PORTABLE_TARGET_AVX2 SimdType<AVX2>::loadAndConvertToFloat(float const* data){
    return _mm256_loadu_ps(data);
}

template<>
typename SimdType<AVX512>::register_t PORTABLE_TARGET_AVX512 SimdType<AVX512>::loadAndConvertToFloat(float const* data){
...
template<>
typename SimdType<SSE>::register_t PORTABLE_TARGET_SSE SimdType<SSE>::loadAndConvertToFloat(E4M3 const* data){
    ...
    const __m128i subnormal_exponent_lookup = _mm_set_epi8(0xFF,0x77,0x00,0x77,0x00,0x77,0x00,0x77,0x00,0x76,0x00,0x76,0x00,0x75,0x00,0x00);
    const __m128i subnormal_mantissa_lookup = _mm_set_epi8(0x01,0x06,0x00,0x04,0x00,0x02,0x00,0x00,0x00,0x04,0x00,0x00,0x00,0x00,0x00,0x00);
    ...
    // normal case
    __m128i normal_exponent = _mm_and_si128(e4m3_vector_32,_mm_set1_epi32(0b00011110));
...
typename SimdType<AVX2>::register_t PORTABLE_TARGET_AVX2 SimdType<AVX2>::loadAndConvertToFloat(E4M3 const* data){
    ...
    const __m256i subnormal_exponent_lookup = _mm256_castsi128_si256(_mm_set_epi8(0xFF,0x77,0x00,0x77,0x00,0x77,0x00,0x77,0x00,0x76,0x00,0x76,0x00,0x75,0x00,0x00));
    const __m256i subnormal_mantissa_lookup = _mm256_castsi128_si256(_mm_set_epi8(0x01,0x06,0x00,0x04,0x00,0x02,0x00,0x00,0x00,0x04,0x00,0x00,0x00,0x00,0x00,0x00));

    __m256i subnormal_lookup_idx = _mm256_srli_epi64(_mm256_castsi128_si256(e4m3_vector), 4);
    ...
    // normal case
    __m256i normal_exponent = _mm256_and_si256(e4m3_vector_32,_mm256_set1_epi32(0b00011110));
...
template<>
typename SimdType<AVX512>::register_t PORTABLE_TARGET_AVX512 SimdType<AVX512>::loadAndConvertToFloat(E4M3 const* data){

    __m128i e4m3_vector = _mm_lddqu_si128(reinterpret_cast<const __m128i*>(data));
    __m512i e4m3_vector_32 = _mm512_cvtepu8_epi32(e4m3_vector);

    // Check if e4m3_vector is all zeros or all ones
    __mmask16 is_subnormal = _mm512_cmpeq_epi32_mask(_mm512_and_si512(e4m3_vector_32, _mm512_set1_epi32(0b00011110)),_mm512_setzero_si512());
    __mmask16 is_nan = _mm512_cmpeq_epi32_mask(_mm512_and_si512(e4m3_vector_32, _mm512_set1_epi32(0b11111110)), _mm512_set1_epi32(0b11111110));
    ...
    // normal case
    __m512i normal_exponent = _mm512_and_si512(e4m3_vector_32,_mm512_set1_epi32(0b00011110));
...
template <typename Func, typename... Args>
std::vector<float> PORTABLE_TARGET_SSE SimdType<SSE>::registerToVectorWrapper(Func&& func, Args&&... args) {
...
template <typename Func, typename... Args>
std::vector<float> PORTABLE_TARGET_AVX2 SimdType<AVX2>::registerToVectorWrapper(Func&& func, Args&&... args) {
...
template <typename Func, typename... Args>
std::vector<float> PORTABLE_TARGET_AVX512 SimdType<AVX512>::registerToVectorWrapper(Func&& func, Args&&... args) {
